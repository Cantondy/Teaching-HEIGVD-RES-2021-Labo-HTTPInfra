...
This function will be executed when the JQuery library is loaded because is called used the variable $, it sends an Ajax request to fetch the JSON data provided by our node.js server which provides a list of animals in JSON format.
It will then take the name of the first animal in the list and insert it inside the HTML tag that contains the **class="skills"**. The function will be repeated every 2 seconds.
